...
NUM_UPDATES_AC = 3
# qnaf + spg
# directory exp1<env_num><hyper_parameter><try_num><run_num>
    ...
    with tf.Session() as sess:
        for iteration in range(5):
            ...
            naf = NAF(sess, env, LEARNING_RATE, TAU, GAMMA,
                         ...
                         sigma_P_dep = False,
                         det=False, qnaf=True,
                         scope=str(iteration), hn=0,
                         ac=True, sep_V=False)
            naf.run_n_episodes(EXPLORATION_TIME, MAX_EP_STEPS,
                               MINIBATCH_SIZE, explore=True, num_updates=5, num_updates_ac=NUM_UPDATES_AC)
            naf.run_n_episodes(MAX_EPISODES - EXPLORATION_TIME, MAX_EP_STEPS,
                               MINIBATCH_SIZE, explore=False, num_updates=5, num_updates_ac=NUM_UPDATES_AC)
